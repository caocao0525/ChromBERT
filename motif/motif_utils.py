...
def motifs_init_gen(save_file_dir, pos_seqs, neg_seqs, motifs, p_adjust = 'fdr_bh', alpha = 0.05, verbose=True, allow_multi_match=False, **kwargs):
        ...
        for motif, instances in merged_motif_seqs.items():
            ...
            with open(save_file_dir+'/motif_{}_{}.txt'.format(motif, len(instances['seq_idx'])), 'w') as f:
                for seq in instances['seqs']:
                    f.write(seq+'\n')
       #    # make weblogo   ################################# commented out by Lee
