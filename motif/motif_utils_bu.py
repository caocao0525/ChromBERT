...
def find_high_attention(score, min_len=5, **kwargs):
   ...
   # print("length of score list:" , len(score))
...
#    ########### test by Lee 
#    if motif_regions.any(): # (if not empty)
#        print("motif region index", motif_regions)
##        motif_regions_lst=[]
#        for coord in motif_regions:
#            motif_region_score=score[coord[0]:coord[-1]]
#            print(motif_region_score)
#            print("mean of region score: ", np.mean(motif_region_score))
##            motif_regions_lst.append(motif_region_score)
##        with open("./temp/test_motif_region_scores.txt","w") as g:
##            for sublst in motif_regions_lst:
##                g.write(','.join(map(str,sublst)))
##
#    ##########
...
def motif_analysis(pos_seqs,
    ...
    **kwargs -- other input arguments, may include:
    ...
    ####### Extract score dictionary (added by Lee Aug. 2023)#########
    motif_seq_n_score = {}
    ##################################################################
    ...
    for i, score in enumerate(pos_atten_scores):
        ...
        for motif_idx in motif_regions:
            ...
            ########### Extract score and seq (added by Lee Aug. 2023)##############
            
            motif_score = score[motif_idx[0]:motif_idx[1]]
            print("-------------------------------------------------")
            print("idx; ", motif_idx)
            print("seq: ", seq)
            print("motif_score: ", motif_score)
            if seq  not in motif_seq_n_score:
               motif_seq_n_score[seq]= {'seq_idx':[i],
                       'atten_region_pos':[(motif_idx[0],motif_idx[1])],
                       'score':[motif_score]}
            else:
                motif_seq_n_score[seq]['seq_idx'].append(i)
                motif_seq_n_score[seq]['atten_region_pos'].append((motif_idx[0],motif_idx[1]))
                motif_seq_n_score[seq]['score'].append(motif_score)

    df_motif_score=pd.DataFrame(motif_seq_n_score)
    df_motif_score.to_csv("test_extract_df.csv",index=False)
    ...
    #with open("./temp/test_motif_seqs.txt", "w") as f:
    #    for item in motif_seqs.keys():
    #        f.write('%s\n' %item)
    ###############################################################
    ...
    #cp2_merged_df.to_csv("cp2_merged_df.csv", index=False)
    ####################################################################
    ...
    #merged_motif_df.to_csv("cp3_merged_df.csv",index=False)
    ###################### 

    #####################
    #print("#################Check point 4")
    #print("motifs before filtering is saved as dataframe at {}".format(save_file_dir))
    ######################
    return merged_motif_seqs
